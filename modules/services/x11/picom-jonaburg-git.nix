...
let
  ...
  cfg = config.services.my-service.picom;
  opt = options.services.my-service.picom;
  ...
  mkValueString = v:
    if types.bool.check v then boolToString v
      ...
      throw ''
        invalid expression used in option services.my-service.picom.settings:
        ${v}
      '';
...
in
{
  ...
  options.services.my-service.picom = {
    enable = mkOption {
      type = types.bool;
      default = false;
      description = ''
    ...
    vSync = mkOption {
      ...
      apply = x:
        let
          res = x != "none";
          msg = "The type of services.my-service.picom.vSync has changed to bool:"
  ...
  config = mkIf cfg.enable {
    services.my-service.picom.settings = mkDefaultAttrs {
      # fading
      fading = cfg.fade;
    ...
    systemd.user.services.my-service.picom = {
    ...
    environment.systemPackages = [ pkgs.picom-jonaburg-git ];
  };
